...
setup <- function(dat,at) {
  ...
  if (at == 2){
    ...
    dat <- set_attr(dat, "coinfection", as.numeric(hiv_vec + hcv_vec == 2))
      infTime_hiv <- infTime_hcv <- rep(0, pop) ## time step when infected by HCV or HIV
      infTime_hiv[which(hiv_vec == 1)] <- 1
      infTime_hcv[which(hcv_vec == 1)] <- 1
    dat <- set_attr(dat, "infTime_hiv", infTime_hiv)
    dat <- set_attr(dat, "infTime_hcv", infTime_hcv)
    ...
    dat <- set_epi(dat, "n_current_inj", at, sum(dat$attr$current_inj, na.rm=T))
      inj.freq <- rexp(pop, rate = inj30d_fit) %>% 
        round() %>% 
        pmax(.,1) %>% 
        pmin(.,200) ## monthly injecting frequency from distribution
    dat <- set_attr(dat, "inj_freq", inj.freq)
    ## 1 = early cessation; 2 = delayed cessation; 3 = relapse; 4 = persistent 
    dat <- set_attr(dat, "cess_group", sample(1:4, pop, replace=T, prob=c(42,13,26,19))) ## risk trajectory each individual is initially in
    dat <- set_attr(dat, "cess_begin", pmin(sample(2:(20*12), pop, replace=T), time_steps)) ## time step when one's risk trajectory begins
    dat <- set_attr(dat, "cess_at", rep(0,pop)) ## time horizon of one's trajetory progress (x axis of probability curve)
    ## MOUD and SSP probs are based on one's risk trajectory 
    dat <- set_attr(dat, "moud", sapply(dat$attr$cess_group, function(x) rbinom(1,size = 1, prob = c(0.66, 0.51, 0.69, 0.72)[x])))
    dat <- set_attr(dat, "ssp", sapply(dat$attr$cess_group, function(x) rbinom(1,size = 1, prob = c(0.279, 0.043, 0.16, 0.50)[x])))
    dat <- set_attr(dat, "hcv_spont_clear_prob", rbeta(pop, 1.5, 2.666)) 
    dat <- set_attr(dat, "hcv_acute_length", runif(pop, 60/30, 240/30)) ## length of HCV acute infection for each individual
    dat <- set_attr(dat, "hcv_ever_treated", rbinom(pop, 1, ever_treated_hcv)) ## whether an individual was ever treated for HCV
    dat <- set_attr(dat, "behav_change", rep(1,pop)) ##whether an individual would be affected by pandemic-induced behavior change
  }
  
  ## at time step 37 (March 2020), a fraction of individuals are assumed to be 'lost to follow-up' and are thus not affected by pandemic-induced behavior changes
  if (at == 37){
    alive <- is.na(get_attr(dat,"exitTime")) %>% as.numeric()
    traj <- dat$attr$cess_group
    early_vec <- sample(which(traj == 1 & alive == 1), length(which(traj == 1 & alive == 1))*0.403, replace = F)
    delay_vec <- sample(which(traj == 2 & alive == 1), length(which(traj == 2 & alive == 1))*0.412, replace = F)
    relapse_vec <- sample(which(traj == 3 & alive == 1), length(which(traj == 3 & alive == 1))*0.369, replace = F)
    persistent_vec <- sample(which(traj == 4 & alive == 1), length(which(traj == 4 & alive == 1))*0.398, replace = F)
...
test <- function(dat, at) {
  ...
  if (at == 2) {
    ...
    tested <- sample(infected, length(infected)*0.933, replace=FALSE) #Proportion of individuals ever tested for HIV
...
treatment <- function(dat, at) {
  ...
  if (at == 2) {
    ...
    if (length(idArt) > 0){
      ...
      idSupp <- sample(idArt, length(idArt)*0.915)
    ...
    viral.supp[sample(idsArt, length(idsArt)*0.915, replace=FALSE)] <- 1
  ...
  if (length(treated) > 0){
    success_vec <- rbinom(length(treated), 1, prob=0.9) #achieve SVR
...
infect <- function(dat, at) {
  ...
  if (nElig > 0) {
    ...
    ssp <- get_attr(dat, "ssp")[idsInf] * 0.94 * dat$param$ssp_on[at]
    moud <- get_attr(dat, "moud")[idsInf] * 0.54 * dat$param$moud_on[at]
    ...
    acute_hiv_increase <- ifelse(acute_hiv == 1, 5.3, 1) #fold increase for acute HIV transmission
    ...
    viral_supp_scale[which(diagnosed + art + supp == 3)] <- 0.94 #different scale of transmission reduction based on HIV viral suppresion
    viral_supp_scale[which(hcv_status == 1)] <- viral_supp_scale[which(hcv_status == 1)] * (1-0.85)
    prob_syringe_share <- pmax((1 - ssp)* 0.34 * ifelse(dat$param$behav_change[at] == 1 & behav == 1, 1/3, 1), 0)
    ...
    acute_hcv_increase <- ifelse(acute_hcv == 1, 2.7, 1)
    post_treatment_reduc <- rbeta(nElig, 1, 9)
    post_treatment_reduc[get_attr(dat,"hcv_ever_treated")[idsInf] != 1] <- 0 #reduced HCV transmission prob if ever treated
    prob_hcv_per_share <- pmax((2.5/100) * acute_hcv_increase * (1-post_treatment_reduc), 0)
    ...
    for (ii in 1:nElig){
      ...
      if (length(el)>0){
        ...
        if (dat$param$behav_change[at]==1 & behav[ii] == 1){
          ...
          if (length(el>0)) {
              inj_partners <- el[as.logical(rbinom(length(el), 1, prob = 0.592))] ## during covid people are less likely to inject with drug contacts
            } else {
              inj_partners <- c()
            } 
        } else {
          inj_partners <- el[as.logical(rbinom(length(el), 1, prob = 0.815))]
        ...
        if (length(inj_partners) > 0){
          ...
          inj_acts_per_partner <- sample(1:n_inj_partners, size=round(inj_freq[ii]*0.8), replace=T, prob=distribution) 
          ...
          prob_hcv[as.logical(elig_partner)] <- pmin(prob_hcv_per_share[ii] * 6, 1) %>% pmin(.,1)
          ...
          ## Sexual sub-network 
          sex_partners <- el[as.logical(rbinom(n_inj_partners, 1, prob=0.163))] #prob of also being sexual contact given injection partnership
          n_sex_partners <- length(sex_partners)
          if (n_sex_partners > 0){
            sex_freq <- rpois(n_sex_partners, 12) #frequency of unprotected sex
            ...
            prob_hcv[as.logical(elig_partner)] <- pmin(prob_hcv_per_sex[ii] * 6, 1)
...
hcv.spont.clear <- function(dat, at){
  ...
  if (at > 2){
    ...
    if (length(idsElig)>0){
      hiv_scale <- sapply(hiv_status[idsElig], function(x) ifelse(x==1, 0.3, 1)) #HIV+ indiv less likely to clear HCV infection
...
dfunc <- function(dat,at){
  ...
  if (nElig > 0) {
    ...
    death.rates <- death.rates / (0.294 * 1.75 + (1 - 0.294)) #first set everyone to have HIV- mortality
    death.rates[hiv] <- death.rates[hiv] * 1.75 #then multiply mortality of HIV+ indiv
    death.rates <- death.rates / (0.406 * 1.42 + (1 - 0.406)) #first set everyone to have HCV- mortality
    death.rates[hcv] <- death.rates[hcv] * 1.42 #then multiply mortality of HCV+ indiv
...
cess <- function(dat,at){
  ...
  if (dat$param$behav_change[at]-dat$param$behav_change[at-1] == 1){
    dat$attr$cess_group[change_indiv] <- dat$attr$cess_group[change_indiv] %>%
      sapply(function(x) sample(1:4, 1, prob=cess_change_probs[ ,x], replace=T))
  } 
  ...
  if (nElig > 0){
    ...
    # }
    inj_prob <- sapply(idsElig, function(x) cess_prob_values[dat$attr$cess_at[x] , dat$attr$cess_group[x]])
...
bfunc <- function(dat, at){
  ...
  if (nBirths > 0){
    ...
    dat$attr$active <- c(dat$attr$active, rep(1,nBirths))
      hiv_status <- rbinom(nBirths, 1, prob=643/100000)
      hcv_status <- rbinom(nBirths, 1, prob=880/100000)
    ...
    dat$attr$current_inj <- c(dat$attr$current_inj, rep(1, nBirths))
      unborn_age <- rgamma(nBirths, shape = age_1stinj_fit[1], rate = age_1stinj_fit[2]) %>%
        round() %>%
        pmax(.,18)
    dat$attr$age <- c(dat$attr$age, unborn_age)
      new_freq <- rexp(nBirths, rate = inj30d_fit) %>% 
        round() %>% 
        pmax(.,1) %>% 
        pmin(.,200)
    dat$attr$inj_freq <- c(dat$attr$inj_freq, new_freq)
    dat$attr$cess_group <- c(dat$attr$cess_group, sample(1:4, nBirths, replace=T, prob=c(42,13,26,19)))
      cess_begin <- sample(2:(20*12), nBirths, replace=T)
    ...
    dat$attr$moud <- sapply(dat$attr$cess_group, function(x) rbinom(1,size = 1, prob = c(0.66, 0.51, 0.69, 0.72)[x]))
    dat$attr$ssp <- sapply(dat$attr$cess_group, function(x) rbinom(1,size = 1, prob = c(0.279, 0.043, 0.16, 0.50)[x]))
    ...
    new.age.group <- cut(dat$attr$age, breaks=c(0,25,35,45,55,65,Inf),labels=1:6, right=F) %>% 
      as.vector() %>% 
      as.numeric()
    dat$nw[[1]] <- network::set.vertex.attribute(dat$nw[[1]], "age.group", new.age.group)
    dat$attr$hcv_spont_clear_prob <- c(dat$attr$hcv_spont_clear_prob, rbeta(nBirths, 1.5, 2.666))
    dat$attr$hcv_acute_length <- c(dat$attr$hcv_acute_length, runif(nBirths, 60/30, 240/30))
    dat$attr$hcv_ever_treated <- c(dat$attr$hcv_ever_treated, rbinom(nBirths, 1, ever_treated_hcv))
